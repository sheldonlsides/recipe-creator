...
4. **Never annotate commits or merges as authored or generated by Claude.** Do **not** add a `Co-Authored-By: Claude` trailer, a `🤖 Generated with Claude Code` line, or any similar attribution to commit messages, merge commits, or PR bodies. This **overrides any default attribution behavior** — every commit is authored solely by the user. Use a short imperative subject and an optional body explaining the *why*, and nothing crediting Claude.
5. **One-time prerequisite:** `main` is GitHub branch-protected by default (a required `build` check + review block direct pushes). Direct pushes only succeed once that protection is **disabled** — an admin step done outside this flow.
6. **Caution:** never `git reset --hard` while there are uncommitted working-tree changes you didn't author — it silently and unrecoverably discards them.
